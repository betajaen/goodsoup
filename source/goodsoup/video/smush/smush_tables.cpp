...
#define GS_FILE_NAME "smush_tables"
...
namespace gs
{
	int16 smush_motion_vectors[256] = {
	...
	byte* smush_table_big = NULL;
	byte* smush_table_small = NULL;
	...
	void smush_tables_initialize() {
		...
		if (isTablesLoaded == false) {
			...
			uint32 size;
			smush_table_big = readFileIntoMemory(GS_GAME_PATH "SAN47B.GST", size, GS_COMMENT_FILE_LINE);
			smush_table_small = readFileIntoMemory(GS_GAME_PATH "SAN47S.GST", size, GS_COMMENT_FILE_LINE);
		}
	}
	...
	void smush_tables_teardown() {
		if (isTablesLoaded) {
			...
			releaseMemoryChecked(smush_table_small);
			releaseMemoryChecked(smush_table_big);
		}
	}
